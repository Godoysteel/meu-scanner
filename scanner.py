...
st.markdown("""
    ...
    .main-title { color: #ffffff; font-size: 32px; font-weight: bold; text-align: center; border-bottom: 2px solid #10b981; padding-bottom: 10px; }
...
def play_alert():
    # Usando um som de sistema padrão (Notification)
...
# --- SIDEBAR (CONFIGURAÇÕES + JOGOS POPULARES) ---
st.sidebar.title("Configurações")
som_ativo = st.sidebar.toggle("🔔 Ativar Alerta Sonoro", value=False)

if som_ativo:
    st.sidebar.success("Som ativado! (Certifique-se que o volume está alto)")
    # Toca um som curto ao ativar para "pedir permissão" ao navegador
    if st.session_state.get('first_run', True):
        play_alert()
        st.session_state['first_run'] = False

st.sidebar.divider()
st.sidebar.subheader("📺 Jogos Populares (Transmissão)")

# Lista de jogos populares - Edite aqui diariamente
populares = [
    {"jogo": "Real Madrid vs Barça", "hora": "16:00", "canal": "Star+"},
    {"jogo": "Man. City vs Arsenal", "hora": "Ao Vivo", "canal": "ESPN"},
    {"jogo": "Flamengo vs Palmeiras", "hora": "18:30", "canal": "Premiere"}
]

for p in populares:
    st.sidebar.markdown(f"""
        <div class="card-transmissao">
            <span style="color:#ffaa00; font-size:11px;">{p['hora']}</span><br>
            <span style="color:white; font-size:14px; font-weight:bold;">{p['jogo']}</span><br>
            <span style="color:#10b981; font-size:12px;">📺 {p['canal']}</span>
        </div>
    """, unsafe_allow_html=True)
...
try:
    ...
    if not jogos_lista:
        st.info("Aguardando jogos no padrão GDscanner (0x0 | 5' a 28')...")
